...
var arraypast = allevent.filter(e => GetApiData.currentDate  > e.date)
...
function filterArray() {
    ...
    if (checkboxSelected.length > 0 && textSearch !== "") {
        checkboxSelected.map(lascategorias => {
            info.push(...arraypast.filter(sala => sala.name.toLowerCase().includes(textSearch.trim().toLowerCase())  &&
                sala.category == lascategorias))
        })
    }
    else if (checkboxSelected.length > 0 && textSearch === "") {
        checkboxSelected.map(lascategorias => info.push(...arraypast.filter(sala => sala.category == lascategorias)))
    }
    else if (checkboxSelected.length == 0 && textSearch !== "") {
        info.push(...arraypast.filter(sala => sala.name.toLowerCase().includes(textSearch.trim().toLowerCase())))
    }
    else { info.push(...arraypast) }
